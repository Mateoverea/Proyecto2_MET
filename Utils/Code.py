...
from Technicalanalysis.Main import Order, Order2
...
def test_strategy(datatrain, sl_val, tp_val, sma_s, sma_l, win_s, win_f, win_si, rsi_v,
    ...
    for i, row in datatrain.iterrows():
        ...
        # Buy
        buy_signals = [row.buy_signal_sma, row.buy_signal_ema, row.buy_signal_rsi, row.buy_signal_bollinger,
                       row.buy_signal_MACD]
        ...
        # Sell
        sell_signals = [row.sell_signal_sma, row.sell_signal_ema, row.sell_signal_rsi, row.sell_signal_bollinger,
                        row.sell_signal_MACD]
...
mpf.plot(datatrain, type='line', style='yahoo', title="Apple Candlestick Chart", ylabel="Price")
